...
num_seeds_y = 80 #Up to 5000m (3 seeds every vertical grid point)
num_seeds_x = 475 #One for each x gridpoint
time_steps = ds.th[:,0,0,0].size-2 #Number of time steps to run trajectories back
...
zpos_grid = np.zeros((time_steps, num_seeds_x, num_seeds_y))
change_zs = np.zeros((time_steps, num_seeds_x, num_seeds_y))

#Variable
var_name1 = 'th'
variable1 = np.zeros((time_steps, num_seeds_x, num_seeds_y))
...
#Add chunck of nan's to array so that the domain is always filled with parcels
...
zpos_grid = np.append(zpos_grid, n, axis = 1)
change_zs = np.append(change_zs, n, axis = 1)
variable1 = np.append(variable1, n, axis = 1)
...
try:
    ...
    zpos_terrain[0,:,:] = np.reshape(interpolate.interpn((z,y,x), zh, coor_terrain[:-num_seeds_y], method='linear', bounds_error=False, fill_value= 0), (2*num_seeds_x-1,num_seeds_y))
...
#    levels_ticks = np.arange(np.int(np.nanmin(color[0,:,:]))+0.25,np.int(np.nanmax(color[0,:,:]))-0.24999,0.5)
    ...
    #Plot
    trajectories_xz = plt.scatter(xpos[i,:,:], zpos[i,:,:]*100, c = color[i,:,:], cmap = cmap, norm=matplotlib.colors.BoundaryNorm(levels,cmap.N), s = 2.5, zorder = 5)
    ...
    plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/png_for_gifs/forward_trajectory_1500_everywhere45sec6%d" % zpos[0,0,0] + "00m%03d.png" % i, dpi = 100)
    plt.close(fig)
    print i

#run function to create images
pool = multiprocessing.Pool(20) #number of processors
