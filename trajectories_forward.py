...
for j in range(time_steps-1):
    ...
    xpos[j,j,:] = 800 #Start on left side of domain
    ypos[j,j,:] = np.linspace(0, num_y-1, num_seeds) #Evenly spaced in y
    zpos[j,j,:] = 5 #Height to start trajectories at
    ###############################################################################
    ###############################################################################
...
for t in range(time_steps-1):
    ...
    for j in range(t+1): 
        ...
        #Color variable (theta)
        color[j,t,:] = interpolate.interpn((z,y,x), th, coor, bounds_error=False, fill_value= np.nan)
...
for j in range(t+1):    
    ...
    #Color variable (theta)
    color[j,t+1,:] = interpolate.interpn((z,y,x), th, coor, bounds_error=False, fill_value= np.nan)
...
#    levels = np.linspace(262,270.01,100)
#    levels_ticks = np.arange(262,270.01,0.5) 
    ...
    plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/png_for_gifs/forward_trajectory_45sec_500m_release_test%d" % zpos[0,0,0] + "00m%03d.png" % i, dpi = 200)
    plt.close(fig)
    print i

##Build GIF
os.system('module load imagemagick')
os.system('convert -delay 10 -quality 100 /uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/png_for_gifs/forward_trajectory_45sec_500m_release*.png  /uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/gifs/forward_trajectory_45sec_20ms_500m_release.gif')
