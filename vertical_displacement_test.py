...
###############################################################################
    ...
    #Plot variables
    if j == 1:
        z_disp_plot = plt.contourf(xpos[0,:,xleft:xright], zpos_terrain[0,:,xleft:xright]/zpos_grid[0,:,xleft:xright]*3, mean_z_disp[:,xleft:xright], zlevels, cmap = shifted_cmapz, alpha = 1, zorder = 3, extend = 'both')
    ...
    if j == 2:
        ...
        x_disp_plot = plt.contourf(xpos[0,:,xleft:xright], zpos_terrain[0,:,xleft:xright]/zpos_grid[0,:,xleft:xright]*3, mean_x_disp[:,xleft:xright]*hor_resolution/1000, xlevels,  cmap = shifted_cmapx, alpha = 1, zorder = 3, extend = 'both') #Percentage
        #x_disp_plot = plt.contourf(x1d, y1d, variable1[0,:,:], zlevels,  cmap = cmap_dth, alpha = 0.5, zorder = 4) #Percentage
    
    #Plot Terrain
    terrain = plt.plot(x1d[xleft:xright], z[xleft:xright]-2, c = 'slategrey', linewidth = 6)
    
    #Plot Lake
    lake = ds.xland[0,ymid,:].values
    lake[lake == 1] = np.nan
    lake_plt = plt.plot(x1d[xleft:xright], lake[xleft:xright]-3, c = 'blue', linewidth = 4, zorder = 5)
